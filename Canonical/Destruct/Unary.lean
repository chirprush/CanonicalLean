...
def forallTelescopeN
  ...
  MetaM α :=
    match types with
    | [] => k [] []
    | t::types' => forallTelescope t fun inputTypes outputType =>
      forallTelescopeN types' fun inputs outputs => k (inputTypes::inputs) (outputType::outputs)
...
mutual
def destructInduct (t : Expr) (builtinCtors : Array Expr) (builtinRec : Expr) : MetaM (Option Isomorphism) := do
  sorry

def destructPi (t : Expr) (inputType : Expr) (outputType : Expr) : MetaM (Option Isomorphism) := do
  sorry

def destruct (t : Expr) : MetaM (Option Isomorphism) := do
  if !(← inferType t).isSort then return .none
  let t ← whnf t
...
#eval show MetaM Unit from (do
  ...
  IO.println $ repr (← extractInfo t).get!
  )
